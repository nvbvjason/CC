...
namespace {
    using RegType = CodeGen::RegisterOperand::Type;
...
void unaryInst(std::vector<std::unique_ptr<Inst>>& insts, const Ir::UnaryInst* irUnary)
{
    ...
    insts.push_back(std::make_unique<MoveInst>(src, dst, getAssemblyType(irUnary->type)));

    insts.push_back(std::make_unique<UnaryInst>(dst, oper, getAssemblyType(irUnary->type)));
}
...
void generateFunCallInst(std::vector<std::unique_ptr<Inst>>& insts, const Ir::FunCallInst* type)
{
    ...
    insts.emplace_back(std::make_unique<MoveInst>(
        std::make_shared<RegisterOperand>(RegisterOperand::Type::AX, 4),
        destination,
...
void pushFunCallArgs(std::vector<std::unique_ptr<Inst>>& insts, const Ir::FunCallInst* funcCall)
{
    ...
    for (; regIndex < funcCall->args.size() && regIndex < registerTypes.size(); ++regIndex) {
        ...
        insts.emplace_back(std::make_unique<MoveInst>(
            src,
            std::make_shared<RegisterOperand>(registerTypes[regIndex], 4),
            getAssemblyType(funcCall->args[regIndex]->type)
        ));
    }
    for (i32 i = funcCall->args.size() - 1; regIndex <= i; --i) {
        ...
        if (src->kind == Operand::Kind::Imm ||
            ...
            insts.emplace_back(std::make_unique<MoveInst>(
                src, std::make_shared<RegisterOperand>(RegisterOperand::Type::AX, 4),
                getAssemblyType(funcCall->args[i]->type)
                ));
            insts.emplace_back(std::make_unique<PushInst>(
                std::make_shared<RegisterOperand>(RegisterOperand::Type::AX, 8))
            );
        }
    }
}
