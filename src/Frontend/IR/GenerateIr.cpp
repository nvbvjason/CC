...
std::unique_ptr<TopLevel> GenerateIr::topLevelIr(const Parsing::Declaration& decl)
{
    ...
    switch (decl.kind) {
        case Kind::FuncDecl: {
            const auto funcDecl = dyn_cast<const Parsing::FunDecl>(&decl);
            if (funcDecl->body == nullptr)
                return nullptr;
            return functionIr(*funcDecl);
        }
        case Kind::VarDecl: {
            const auto varDecl = dyn_cast<const Parsing::VarDecl>(&decl);
...
{
    if (defined) {
        const auto singleInit = dyn_cast<const Parsing::SingleInit>(varDecl.init.get());
...
std::unique_ptr<TopLevel> GenerateIr::functionIr(const Parsing::FunDecl& parsingFunction)
{
    ...
    const auto funcType = dyn_cast<const Parsing::FuncType>(parsingFunction.type.get());
...
void GenerateIr::genBlockItem(const Parsing::BlockItem& blockItem)
{
    ...
    switch (blockItem.kind) {
        case Kind::Declaration: {
            const auto decl = dyn_cast<const Parsing::DeclBlockItem>(&blockItem);
            genDeclaration(*decl->decl);
            break;
        }
        case Kind::Statement: {
            const auto stmtBlockItem = dyn_cast<const Parsing::StmtBlockItem>(&blockItem);
...
void GenerateIr::genForInit(const Parsing::ForInit& forInit)
{
    if (forInit.kind == Parsing::ForInit::Kind::Declaration) {
        const auto decl = dyn_cast<const Parsing::DeclForInit>(&forInit);
        genDeclaration(*decl->decl);
        return;
    }
    if (forInit.kind == Parsing::ForInit::Kind::Expression) {
        const auto expr = dyn_cast<const Parsing::ExprForInit>(&forInit);
        if (expr->expression)
...
void GenerateIr::genDeclaration(const Parsing::Declaration& decl)
{
    ...
    if (varDecl->init == nullptr)
        return;
    const auto init = dyn_cast<const Parsing::SingleInit>(varDecl->init.get());
...
void GenerateIr::genStmt(const Parsing::Stmt& stmt)
{
    ...
    switch (stmt.kind) {
        case Kind::If: {
            const auto ifStmt = dyn_cast<const Parsing::IfStmt>(&stmt);
            genIfStmt(*ifStmt);
            break;
        }
        case Kind::Return: {
            const auto returnStmt = dyn_cast<const Parsing::ReturnStmt>(&stmt);
            genReturnStmt(*returnStmt);
            break;
        }
        case Kind::Expression: {
            const auto stmtExpr = dyn_cast<const Parsing::ExprStmt>(&stmt);
            genInst(*stmtExpr->expr);
            break;
        }
        case Kind::Goto: {
            const auto gotoStmt = dyn_cast<const Parsing::GotoStmt>(&stmt);
            genGotoStmt(*gotoStmt);
            break;
        }
        case Kind::Compound: {
            const auto compoundStmtPtr = dyn_cast<const Parsing::CompoundStmt>(&stmt);
            genCompoundStmt(*compoundStmtPtr);
            break;
        }
        case Kind::Break: {
            const auto breakStmtPtr = dyn_cast<const Parsing::BreakStmt>(&stmt);
            genBreakStmt(*breakStmtPtr);
            break;
        }
        case Kind::Continue: {
            const auto continueStmtPtr = dyn_cast<const Parsing::ContinueStmt>(&stmt);
            genContinueStmt(*continueStmtPtr);
            break;
        }
        case Kind::Label: {
            const auto labelStmtPtr = dyn_cast<const Parsing::LabelStmt>(&stmt);
            genLabelStmt(*labelStmtPtr);
            break;
        }
        case Kind::Case: {
            const auto caseStmtPtr = dyn_cast<const Parsing::CaseStmt>(&stmt);
            genCaseStmt(*caseStmtPtr);
            break;
        }
        case Kind::Default: {
            const auto defaultStmtPtr = dyn_cast<const Parsing::DefaultStmt>(&stmt);
            genDefaultStmt(*defaultStmtPtr);
            break;
        }
        case Kind::DoWhile: {
            const auto doWhileStmtPtr = dyn_cast<const Parsing::DoWhileStmt>(&stmt);
            genDoWhileStmt(*doWhileStmtPtr);
            break;
        }
        case Kind::While: {
            const auto whileStmtPtr = dyn_cast<const Parsing::WhileStmt>(&stmt);
            genWhileStmt(*whileStmtPtr);
            break;
        }
        case Kind::For: {
            const auto forStmtPtr = dyn_cast<const Parsing::ForStmt>(&stmt);
            genForStmt(*forStmtPtr);
            break;
        }
        case Kind::Switch: {
            const auto switchStmtPtr = dyn_cast<const Parsing::SwitchStmt>(&stmt);
...
std::unique_ptr<ExprResult> GenerateIr::genInst(const Parsing::Expr& parsingExpr)
{
    ...
    switch (parsingExpr.kind) {
        case ExprKind::Cast: {
            const auto castExpr = dyn_cast<const Parsing::CastExpr>(&parsingExpr);
            return genCastInst(*castExpr);
        }
        case ExprKind::Var: {
            const auto varExpr = dyn_cast<const Parsing::VarExpr>(&parsingExpr);
            return genVarInst(*varExpr);
        }
        case ExprKind::Constant: {
            const auto constExpr = dyn_cast<const Parsing::ConstExpr>(&parsingExpr);
            return genConstInst(*constExpr);
        }
        case ExprKind::Unary: {
            const auto unaryParsingPtr = dyn_cast<const Parsing::UnaryExpr>(&parsingExpr);
            return genUnaryInst(*unaryParsingPtr);
        }
        case ExprKind::Binary: {
            const auto binaryParsingPtr = dyn_cast<const Parsing::BinaryExpr>(&parsingExpr);
            return genBinaryInst(*binaryParsingPtr);
        }
        case ExprKind::Assignment: {
            const auto assignmentExpr = dyn_cast<const Parsing::AssignmentExpr>(&parsingExpr);
            return genAssignInst(*assignmentExpr);
        }
        case ExprKind::Ternary: {
            const auto ternaryExpr = dyn_cast<const Parsing::TernaryExpr>(&parsingExpr);
            return genTernaryInst(*ternaryExpr);
        }
        case ExprKind::FunctionCall: {
            const auto funcCallExpr = dyn_cast<const Parsing::FuncCallExpr>(&parsingExpr);
            return genFuncCallInst(*funcCallExpr);
        }
        case ExprKind::Dereference: {
            const auto dereferenceExpr = dyn_cast<const Parsing::DereferenceExpr>(&parsingExpr);
            return genDereferenceInst(*dereferenceExpr);
        }
        case ExprKind::AddrOf: {
            const auto addrOfExpr = dyn_cast<const Parsing::AddrOffExpr>(&parsingExpr);
...
std::shared_ptr<Value> GenerateIr::genInstAndConvert(const Parsing::Expr& parsingExpr)
{
    ...
    switch (result->kind) {
        case ExprResult::Kind::PlainOperand: {
            const auto plainOperand = dyn_cast<const PlainOperand>(result.get());
            return plainOperand->value;
        }
        case ExprResult::Kind::DereferencedPointer: {
            const auto dereferencedPointer = dyn_cast<const DereferencedPointer>(result.get());
...
std::unique_ptr<ExprResult> GenerateIr::genUnaryPostfixInst(const Parsing::UnaryExpr& unaryExpr)
{
    ...
    switch (original->kind) {
        case ExprResult::Kind::PlainOperand: {
            const auto plainOperand = dyn_cast<const PlainOperand>(original.get());
            insts.emplace_back(std::make_unique<CopyInst>(plainOperand->value, originalForReturn, type));
            insts.emplace_back(std::make_unique<BinaryInst>(
                oper, originalForReturn, one, tempNew, type));
            insts.emplace_back(std::make_unique<CopyInst>(tempNew, plainOperand->value, type));
            return std::make_unique<PlainOperand>(originalForReturn);
        }
        case ExprResult::Kind::DereferencedPointer: {
            const auto dereferencedPointer = dyn_cast<const DereferencedPointer>(original.get());
            const auto derefValue = std::make_shared<ValueVar>(Identifier(makeTemporaryName()), type);
            insts.emplace_back(std::make_unique<LoadInst>(dereferencedPointer->ptr, derefValue, type));
            insts.emplace_back(std::make_unique<CopyInst>(derefValue, originalForReturn, type));
            insts.emplace_back(std::make_unique<BinaryInst>(
                oper, originalForReturn, one, tempNew, type));
            insts.emplace_back(std::make_unique<StoreInst>(tempNew, dereferencedPointer->ptr, type));
            return std::make_unique<PlainOperand>(originalForReturn);
        }
    }
    return std::make_unique<PlainOperand>(originalForReturn);
...
std::unique_ptr<ExprResult> GenerateIr::genUnaryPrefixInst(const Parsing::UnaryExpr& unaryExpr)
{
    ...
    switch (original->kind) {
        case ExprResult::Kind::PlainOperand: {
            const auto plainOperand = dyn_cast<const PlainOperand>(original.get());
            insts.emplace_back(std::make_unique<BinaryInst>(
                operation, plainOperand->value, one, temp, type));
            insts.emplace_back(std::make_unique<CopyInst>(
                temp, plainOperand->value, type));
        ...
        case ExprResult::Kind::DereferencedPointer: {
            ...
            insts.emplace_back(std::make_unique<StoreInst>(
                temp, dereferencedPointer->ptr, type));
            return std::make_unique<PlainOperand>(temp);
        }
    }
    std::abort();
...
std::unique_ptr<ExprResult> GenerateIr::genAssignInst(const Parsing::AssignmentExpr& assignmentExpr)
{
    ...
    switch (lhs->kind) {
        case ExprResult::Kind::PlainOperand: {
            const auto plainOperand = dyn_cast<const PlainOperand>(lhs.get());
            insts.emplace_back(std::make_unique<CopyInst>(
                rhs, plainOperand->value, assignmentExpr.type->type));
            return std::make_unique<PlainOperand>(plainOperand->value);
        }
        case ExprResult::Kind::DereferencedPointer: {
            const auto dereferencedPointer = dyn_cast<const DereferencedPointer>(lhs.get());
            insts.emplace_back(std::make_unique<StoreInst>(
                rhs, dereferencedPointer->ptr, assignmentExpr.type->type));
...
std::unique_ptr<ExprResult> GenerateIr::genTernaryInst(const Parsing::TernaryExpr& ternaryExpr)
{
    ...
    std::shared_ptr<Value> condition = genInstAndConvert(*ternaryExpr.condition);

    insts.emplace_back(std::make_unique<JumpIfZeroInst>(condition, falseLabelName));

    std::shared_ptr<Value> trueValue = genInstAndConvert(*ternaryExpr.trueExpr);
    ...
    std::shared_ptr<Value> falseValue = genInstAndConvert(*ternaryExpr.falseExpr);
...
std::unique_ptr<ExprResult> GenerateIr::genFuncCallInst(const Parsing::FuncCallExpr& funcCallExpr)
{
    ...
    insts.emplace_back(std::make_unique<FunCallInst>(
        Identifier(funcCallExpr.name), std::move(arguments), dst, funcCallExpr.type->type));
    return std::make_unique<PlainOperand>(dst);
}
...
std::unique_ptr<ExprResult> GenerateIr::genAddrOfInst(const Parsing::AddrOffExpr& addrOffExpr)
{
    if (addrOffExpr.reference->kind == Parsing::Expr::Kind::Dereference) {
        const auto deferenceExpr = dyn_cast<const Parsing::DereferenceExpr>(addrOffExpr.reference.get());
        return genInst(*deferenceExpr->reference);
    }
    const std::unique_ptr<ExprResult> inner = genInst(*addrOffExpr.reference);
    switch (inner->kind) {
        case ExprResult::Kind::PlainOperand: {
            const auto plainOperand = dyn_cast<const PlainOperand>(inner.get());
            Identifier dstIden = makeTemporaryName();
        ...
        case ExprResult::Kind::DereferencedPointer: {
            const auto dereferencedPointer = dyn_cast<const DereferencedPointer>(inner.get());
