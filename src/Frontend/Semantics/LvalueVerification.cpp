...
std::vector<Error> LvalueVerification::resolve(const Parsing::Program& program)
{
...
void LvalueVerification::visit(const Parsing::UnaryExpr& unaryExpr)
{
    ...
    if (isNotAnLvalue(unaryExpr.innerExpr->kind)) {
        errors.emplace_back("Unary operation on non lvalue ", unaryExpr.location);
        return;
    }
    ...
    if (innerUnaryExpr->op == Operator::PostFixDecrement || innerUnaryExpr->op == Operator::PostFixIncrement)
        errors.emplace_back("Postfix as inner unary expression ", unaryExpr.location);
}
...
void LvalueVerification::visit(const Parsing::AssignmentExpr& assignmentExpr)
{
    if (assignmentExpr.lhs->kind == Parsing::Expr::Kind::Dot) {
        ...
        if (!isAllowedLValueExprKind(travExpr->kind)) {
            errors.emplace_back("DotExpr of operation on non lvalue ", dotExpr->structuredExpr->location);
            return;
        }
    }
    if (!isAllowedLValueExprKind(assignmentExpr.lhs->kind))
        errors.emplace_back("Assignment onto non LValue type ", assignmentExpr.lhs->location);
...
void LvalueVerification::visit(const Parsing::AddrOffExpr& addrOffExpr)
{
    ...
    if (isNotAnLvalue(addrOffExpr.reference->kind)) {
        errors.emplace_back("Address of operation on non lvalue ", addrOffExpr.reference->location);
        return;
    }
    if (addrOffExpr.reference->kind == Parsing::Expr::Kind::Unary) {
        ...
        if (unaryExpr->op == Operator::PrefixDecrement || unaryExpr->op == Operator::PrefixIncrement) {
            errors.emplace_back("Address of operation on prefix ", addrOffExpr.reference->location);
            return;
        }
        if (unaryExpr->op == Operator::PostFixDecrement || unaryExpr->op == Operator::PostFixDecrement) {
            errors.emplace_back("Address of operation on postfix ", addrOffExpr.reference->location);
            return;
        }
    }
