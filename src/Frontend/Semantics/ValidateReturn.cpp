...
void ValidateReturn::visit(Parsing::FunDecl& funDecl)
{
    ...
    const auto stmtBlockItem = dyn_cast<Parsing::StmtBlockItem>(lastBlockItem.get());
    if (stmtBlockItem->stmt->kind != Parsing::Stmt::Kind::Return) {
        addReturnZero(funDecl);
        return;
    }
    const auto returnStmt = dyn_cast<Parsing::ReturnStmt>(stmtBlockItem->stmt.get());
    const auto funcType = dyn_cast<const Parsing::FuncType>(funDecl.type.get());
